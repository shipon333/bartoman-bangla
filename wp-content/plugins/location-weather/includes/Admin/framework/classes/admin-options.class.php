...
if ( ! class_exists( 'SPLWT_Options' ) ) {
	class SPLWT_Options extends SPLWT_Abstract {
		...
		public function add_options_html() {
			...
			echo '<img class="splwt-setting-logo" src="' . $_menu_icon . '"/><h1>' . $this->args['framework_title'] . '</h1>';
			echo '</div>';
			...
			echo '<input type="submit" name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary splwt-lite-top-save splwt-lite-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save', 'location-weather' ) . '" data-save="' . esc_html__( 'Saving...', 'location-weather' ) . '">';
			echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="splwt_transient[reset_section]" class="button button-secondary splwt-lite-reset-section splwt-lite-confirm" value="' . esc_html__( 'Reset Section', 'location-weather' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'location-weather' ) . '">' : '';
