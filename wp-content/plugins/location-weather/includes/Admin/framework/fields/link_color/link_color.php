 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SPLWT_Field_link_color' ) ) {
	class SPLWT_Field_link_color extends SPLWT_Fields {
		...
		public function render() {
			...
			$color_props = array(
				'color'   => esc_html__( 'Normal', 'location-weather' ),
				'hover'   => esc_html__( 'Hover', 'location-weather' ),
				'active'  => esc_html__( 'Active', 'location-weather' ),
				'visited' => esc_html__( 'Visited', 'location-weather' ),
				'focus'   => esc_html__( 'Focus', 'location-weather' ),
			);
			...
			foreach ( $color_props as $color_prop_key => $color_prop_value ) {
				...
				if ( ! empty( $args[ $color_prop_key ] ) ) {
					...
					echo '<div class="splwt-lite--title">' . esc_attr( $color_prop_value ) . '</div>';
					echo '<input type="text" name="' . esc_attr( $this->field_name( '[' . $color_prop_key . ']' ) ) . '" value="' . esc_attr( $value[ $color_prop_key ] ) . '" class="splwt-lite-color"' . $default_attr . $this->field_attributes() . '/>';
					echo '</div>';
