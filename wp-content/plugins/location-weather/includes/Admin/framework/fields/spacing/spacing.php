	...
	class SPLWT_Field_spacing extends SPLWT_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[all]' ) ) . '" value="' . esc_attr( $value['all'] ) . '"' . $placeholder . ' class="splwt-lite-input-number' . esc_attr( $is_unit ) . '" />';
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . $placeholder . ' class="splwt-lite-input-number' . esc_attr( $is_unit ) . '" />';
					echo ( $unit ) ? '<span class="splwt-lite--label splwt-lite--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
