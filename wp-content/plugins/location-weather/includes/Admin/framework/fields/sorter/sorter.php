 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SPLWT_Field_sorter' ) ) {
	class SPLWT_Field_sorter extends SPLWT_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				...
				array(
					'disabled'       => true,
					'enabled_title'  => esc_html__( 'Enabled', 'location-weather' ),
					'disabled_title' => esc_html__( 'Disabled', 'location-weather' ),
				)
			);
			...
			echo ( $args['disabled'] ) ? '<div class="splwt-lite-modules">' : '';

			echo ( ! empty( $args['enabled_title'] ) ) ? '<div class="splwt-lite-sorter-title">' . esc_attr( $args['enabled_title'] ) . '</div>' : '';
			echo '<ul class="splwt-lite-enabled">';
			...
			if ( $args['disabled'] ) {
				...
				echo '<div class="splwt-lite-modules">';
				echo ( ! empty( $args['disabled_title'] ) ) ? '<div class="splwt-lite-sorter-title">' . esc_attr( $args['disabled_title'] ) . '</div>' : '';
				echo '<ul class="splwt-lite-disabled">';
