	...
	class SPLWT_Field_tabbed extends SPLWT_Fields {
		...
		public function render() {
			...
			foreach ( $this->field['tabs'] as $key => $tab ) {
				...
				$tabbed_icon   = ( ! empty( $tab['icon'] ) ) ? '<i class="splwt-lite--icon ' . esc_attr( $tab['icon'] ) . '"></i>' : '';
				$tabbed_active = ( empty( $key ) ) ? 'splwt-lite-tabbed-active' : '';
			...
			echo '<div class="splwt-lite-tabbed-contents">';
			foreach ( $this->field['tabs'] as $key => $tab ) {

				$tabbed_hidden = ( ! empty( $key ) ) ? ' hidden' : '';

				echo '<div class="splwt-lite-tabbed-content' . esc_attr( $tabbed_hidden ) . '">';
