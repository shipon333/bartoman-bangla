...
class LW_Widget extends \WP_Widget {
	...
	public function form( $instance ) {
		...
		if ( count( $shortcodes ) > 0 ) {
			...
			echo sprintf( '<p><label>%s</label>', __( 'Select Shortcode:', 'location-weather' ) );
			...
			echo sprintf(
				'<p>%1$s <a href="' . admin_url( 'post-new.php?post_type=location_weather' ) . '">%$s</a> %3$s</p>',
				__( 'You did not generate any weather yet.', 'location-weather' ),
				__( 'click here', 'location-weather' ),
				__( 'to generate a new location weather now.', 'location-weather' )
			);
		}
	}
